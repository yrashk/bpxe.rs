...
fn failing_to_establish_sequence_flow_in_process() {
    ...
    assert!(matches!(
        process
            .establish_sequence_flow("no_start", "end", "test", None)
            .unwrap_err(),
    ...
    assert!(matches!(
        process
            .establish_sequence_flow("start", "no_end", "test", None)
            .unwrap_err(),
...
fn failing_to_establish_sequence_flow() {
    let mut start = StartEvent::default();
    let mut end = EndEvent::default();
    assert!(establish_sequence_flow(&mut start, &mut end, "test", None).is_err());
    start.id = Some("start".into());
    assert!(establish_sequence_flow(&mut start, &mut end, "test", None).is_err());
    end.id = Some("end".into());
    assert!(establish_sequence_flow(&mut start, &mut end, "test", None).is_ok());
}
